...
- **Documented three-stage contract for empty `<value/>` handling.** `docs/report-output-spec.md` now carries an "Empty value handling across parser, binding, and display" section that fixes the responsibility split: the parser (`parseCandidateResponses` in `src/qti/assessmentResult.ts`) keeps every empty position in document order and normalizes CRLF/CR to LF; the binding layer (`resolveSubmittedValues` in `src/report/interactionResponses.ts`) keeps the parser's array verbatim, so the legacy ordered `RESPONSE` distribution returns `[""]` at `declarationValueIndex` and the index alignment for subsequent interactions is preserved; the HTML and CSV display layers call the shared `dropEmptyResponseValues` from `src/report/responseValues.ts`, which is the ONLY stage that drops empty strings (strict `length === 0`, no `trim()`). The "Multi-value preservation" section is tightened to point at the new three-stage contract instead of restating it as a single sentence. The interaction `id` canonical-key block and the legacy ordered / direct-match binding rule blocks are unchanged. The "Specifications" section in `README.md` now links to the new spec section. The header comments of `parseCandidateResponses`, `resolveSubmittedValues`, and `dropEmptyResponseValues` are tightened to state which stage each one implements.

- **New E2E test for head-empty and tail-empty legacy ordered `RESPONSE` distribution.** `src/test/legacyOrderedEmptyValues.test.ts` exercises the new `docs/report-output-spec.md` three-stage contract end-to-end through `generateHtmlReportFromFiles`. It reuses the existing `unification-legacy-ordered.qti.xml` item and pins two scenarios that were not directly covered before: a head-empty candidate response (`<value/>` first, then `<value>beta</value>`) where the renderer still binds `RESPONSE_1` to `""` and `RESPONSE_2` to `"beta"`; and a tail-empty candidate response (`<value>alpha</value>` first, then `<value/>`) where `RESPONSE_1` stays bound to `"alpha"` and `RESPONSE_2` is bound to `""` (the HTML layer renders that interaction as `（無回答）`). The test asserts per-interaction `data-interaction-id`, the per-wrapper cloze input count, and the input `value` attribute (or the `（無回答）` label) to lock the parser-keeps / binding-keeps / display-drops contract.

- **Test runner consolidation.** The test runner now lives in `scripts/run-tests.mjs`, with the test discovery logic split out into `src/scripts/discoverTests.ts` (a buildable, tree-shake-free helper that walks `dist/test/*.test.js`). `npm test` builds the project, then spawns `node --test` per discovered test file. This replaces the previous ad-hoc `node --test src/test/*.test.ts` invocation and makes the runner resilient to the `dist/` output layout.

- **Builds always start from a clean `dist/`.** A new `npm run clean` step (`scripts/clean-dist.mjs`) deletes `dist/` before every `tsc` compile, and `npm run build` is now `npm run clean && tsc -p tsconfig.json`, so every caller of `build` (`npm test`, `npm run verify`, `npm start`, and `prepare`) gets a clean build. The clean step is a single cross-platform `fs.rmSync(dist, { recursive: true, force: true })` (no `rm -rf` / PowerShell dependency, succeeds when `dist/` is absent, removes nothing outside `dist/`), mirrored by the unit-tested `cleanDist()` in `src/scripts/cleanDist.ts`. This guarantees that JavaScript built from a since-deleted or renamed source — for example a stale `dist/test/*.test.js` — is never left behind to be re-run or double-counted by the auto-discovering test runner.

- **Test runner has no per-file special-casing.** `scripts/run-tests.mjs` now runs every `*.test.js` under the freshly built `dist/test` via `discoverTests({ distDir })` only; the previous `excludeNames: ['suite.test.js']` workaround (needed when stale artifacts could survive a build) is removed now that `src/test/suite.test.ts` is gone and builds are always clean. The single source of truth for the test list is the `dist/test/*.test.js` auto-discovery; there is no manual test-file list in `package.json` or the runner.

- **`npm run verify` no longer double-builds.** `verify` is now `npm run format && npm run lint && npm test`; the trailing `npm run build` was redundant because `npm test` already performs a clean build, and `dist/` is left as the latest clean build output when `verify` completes.

- **Hardened regression tests for stale artifacts and manual enumeration.** `src/test/test-runner.test.ts` now unit-tests `cleanDist()` (no-throw on a missing directory, full subtree removal, and no deletion outside the target), reproduces the stale-`*.test.js` scenario end-to-end through `cleanDist()` + `discoverTests()`, and asserts — independent of quoting and for both `/` and `\` separators — that neither `package.json`'s `test` script nor `scripts/run-tests.mjs` hand-lists individual test files, calls `node --test` directly, or special-cases `suite.test.js`.
